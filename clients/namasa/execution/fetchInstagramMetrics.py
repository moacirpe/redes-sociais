#!/usr/bin/env python3
"""
Fetch Instagram Metrics — Namasa
...
def loadConfig():
    token   = os.getenv("NAMASA_INSTAGRAM_TOKEN")
    acct_id = os.getenv("NAMASA_INSTAGRAM_ACCOUNT_ID")
    if not token or not acct_id:
        raise EnvironmentError("Defina NAMASA_INSTAGRAM_TOKEN e NAMASA_INSTAGRAM_ACCOUNT_ID no .env")
    return {"token": token, "account_id": acct_id}
...
def fetchProfile(cfg):
    r = buildSession().get(f"{GRAPH_BASE}/{cfg['account_id']}", params={
...
def fetchPosts(cfg, limit=30):
    r = buildSession().get(f"{GRAPH_BASE}/{cfg['account_id']}/media", params={
...
def fetchInsights(cfg, period=30):
    since = int((datetime.utcnow() - timedelta(days=period)).timestamp())
    until = int(datetime.utcnow().timestamp())
    r = buildSession().get(f"{GRAPH_BASE}/{cfg['account_id']}/insights", params={
...
def calcEngagement(post, followers):
    reach = post.get("reach") or followers or 1
    total = sum([post.get("like_count") or 0, post.get("comments_count") or 0,
                 post.get("shares_count") or 0, post.get("saved") or 0])
...
def fetchInstagramMetrics(period=30):
    ...
    profile   = fetchProfile(cfg)
    followers = profile.get("followers_count", 0)
    logger.info(f"@{profile.get('username')} — {followers:,} seguidores")

    posts = fetchPosts(cfg)
    for p in posts:
        p["engagement_rate"] = calcEngagement(p, followers)
    logger.info(f"{len(posts)} posts coletados")

    insights  = fetchInsights(cfg, period)
...
def main():
    ...
    try:
        ...
        if args.save:
            ...
            path = f".tmp/{CLIENT}/instagram_{ts}.json"
