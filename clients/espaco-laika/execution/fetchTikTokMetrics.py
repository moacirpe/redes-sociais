#!/usr/bin/env python3
"""
Fetch TikTok Metrics — Espaço Laika
Credenciais: LAIKA_TIKTOK_ACCESS_TOKEN, LAIKA_TIKTOK_OPEN_ID
...
def loadConfig():
    token = os.getenv("LAIKA_TIKTOK_ACCESS_TOKEN")
    if not token:
        raise EnvironmentError("Defina LAIKA_TIKTOK_ACCESS_TOKEN no .env")
    return {"token": token, "open_id": os.getenv("LAIKA_TIKTOK_OPEN_ID")}
...
def buildSession(token):
    ...
    s.mount("https://", HTTPAdapter(max_retries=Retry(total=3, status_forcelist=[429,500,502,503,504], backoff_factor=1)))
    return s
...
def fetchTikTokMetrics(days=30):
    ...
    topShares  = sorted(videos, key=lambda x: x["shares"], reverse=True)[:5]
...
def main():
    ...
    try:
        ...
        if args.save:
            ...
            path = f".tmp/laika/tiktok_{ts}.json"
