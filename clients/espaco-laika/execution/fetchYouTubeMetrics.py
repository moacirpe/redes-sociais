#!/usr/bin/env python3
"""
Fetch YouTube Metrics — Espaço Laika
Credenciais: LAIKA_YOUTUBE_API_KEY e LAIKA_YOUTUBE_CHANNEL_ID
...
def loadConfig():
    key = os.getenv("LAIKA_YOUTUBE_API_KEY")
    ch  = os.getenv("LAIKA_YOUTUBE_CHANNEL_ID")
    if not key or not ch:
        raise EnvironmentError("Defina LAIKA_YOUTUBE_API_KEY e LAIKA_YOUTUBE_CHANNEL_ID no .env")
...
def fetchChannel(cfg, session=None):
...
def fetchVideoIds(cfg, channel, maxResults=50, session=None):
    session = session or buildSession()
...
def fetchVideoStats(cfg, videoIds, session=None):
    session = session or buildSession()
    videos  = []
...
def fetchYouTubeMetrics(days=30):
    ...
    logger.info("Coletando canal...")
    channel    = fetchChannel(cfg, session=session)
    stats      = channel.get("statistics", {})
    name       = channel["snippet"]["title"]
    logger.info(f"  {name} — {int(stats.get('subscriberCount', 0)):,} inscritos")

    logger.info("Coletando vídeos...")
    videoIds = fetchVideoIds(cfg, channel, session=session)
    ...
    lowCtrCount= len([v for v in videos if v["likes_rate"] < 0.02])
    ...
    return {
        ...
        "analysis": {
            ...
            "low_performance_count": lowCtrCount,
...
def main():
    ...
    try:
        ...
        if args.save:
            ...
            path = f".tmp/laika/youtube_{ts}.json"
