#!/usr/bin/env python3
"""
Fetch Instagram Metrics — Moacir (Pessoal)

Uso:
    python clients/moacir/execution/fetchInstagramMetrics.py --save
    python clients/moacir/execution/fetchInstagramMetrics.py --period 7 --save
"""
...
def loadConfig():
    token   = os.getenv("MOACIR_INSTAGRAM_TOKEN", "").strip("'\"")
    acct_id = os.getenv("MOACIR_INSTAGRAM_ACCOUNT_ID", "").strip("'\"")
    if not token or not acct_id:
        raise EnvironmentError("Defina MOACIR_INSTAGRAM_TOKEN e MOACIR_INSTAGRAM_ACCOUNT_ID no .env")
...
def fetchInstagramMetrics(period=30):
    ...
    try:
        ...
        logger.warning(f"Insights indisponível (token sem permissão): {e}")
        insights = {}
