#!/usr/bin/env python3
"""
Fetch YouTube Metrics — Moacir (Pessoal)
...
def loadConfig():
    key = os.getenv("MOACIR_YOUTUBE_API_KEY")
    ch  = os.getenv("MOACIR_YOUTUBE_CHANNEL_ID")
    if not key or not ch:
        raise EnvironmentError("Defina MOACIR_YOUTUBE_API_KEY e MOACIR_YOUTUBE_CHANNEL_ID no .env")
...
def fetchYouTubeMetrics(days=30):
    ...
    videos   = enrichVideos(fetchVideoStats(cfg, videoIds, session=session))
