#!/usr/bin/env python3
"""
Fetch TikTok Metrics — Moacir (Pessoal)
...
def loadConfig():
    token = os.getenv("MOACIR_TIKTOK_ACCESS_TOKEN")
    if not token:
        raise EnvironmentError("Defina MOACIR_TIKTOK_ACCESS_TOKEN no .env")
    return {"token": token, "open_id": os.getenv("MOACIR_TIKTOK_OPEN_ID")}
...
def fetchAccount(cfg, session=None):
    session = session or buildSession(cfg["token"])
...
def fetchVideos(cfg, days=30, session=None):
    session = session or buildSession(cfg["token"])
...
def fetchTikTokMetrics(days=30):
    ...
    account = fetchAccount(cfg, session=session)
    logger.info(f"@{account.get('username')} — {account.get('followers_count', 0):,} seguidores")

    videos  = enrichVideos(fetchVideos(cfg, days=days, session=session))
